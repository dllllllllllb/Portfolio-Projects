...
TownBuildingUpgrade::TownBuildingUpgrade(sf::RenderWindow& rWindow, Textures* pTextures, Fonts* pFonts) :
	Button(rWindow, pTextures, pFonts),
...
void TownBuildingUpgrade::baseSetUp(const int& positionX, const int& positionY)
{
	...
	//Building sprite
	m_buildingUpgradeSprite.setTexture(m_pTextures->m_baseBuildingUpgradeSprite, true);
	...
	//Image text border
	m_imageTextBorder.setTexture(m_pTextures->m_thinBorder);
